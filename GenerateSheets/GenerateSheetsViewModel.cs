...
namespace TeslaRevitTools.GenerateSheets
{
    public class GenerateSheetsViewModel : INotifyPropertyChanged
    {
        ...
        private List<ViewSheetParameterSerialized> GetSerializedParameterValues()
        {
            ...
            string jsonFilePath = Path.Combine(tslaRevitToolsDirectory, "titleblock_sheet_parameters.json");
        ...
        public void SaveNamingRules()
        {
            string assemblyDirPath = Path.GetDirectoryName(System.Reflection.Assembly.GetExecutingAssembly().Location);
            string tslaRevitToolsDirectory = Directory.GetParent(assemblyDirPath).FullName;
            string jsonFilePath = Path.Combine(tslaRevitToolsDirectory, "naming_rules_v2.json");
        ...
        public void SaveParameterModifiers()
        {
            string assemblyDirPath = Path.GetDirectoryName(System.Reflection.Assembly.GetExecutingAssembly().Location);
            string tslaRevitToolsDirectory = Directory.GetParent(assemblyDirPath).FullName;
            string jsonFilePath1 = Path.Combine(tslaRevitToolsDirectory, "modifier_substring.json");
            string jsonFilePath2 = Path.Combine(tslaRevitToolsDirectory, "modifier_findreplace.json");
        ...
        public void GetSettings()
        {
            ...
            string assemblyDirPath = Path.GetDirectoryName(System.Reflection.Assembly.GetExecutingAssembly().Location);
            string tslaRevitToolsDirectory = Directory.GetParent(assemblyDirPath).FullName;
            string jsonFilePath = Path.Combine(tslaRevitToolsDirectory, "sheetview_settings.json");
            if (File.Exists(jsonFilePath))
            {
                string content = File.ReadAllText(jsonFilePath);
