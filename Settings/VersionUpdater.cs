...
namespace TeslaRevitTools.Settings
{
    public class VersionUpdater
    {
        ...
        public async void CheckForUpdates(SettingsViewModel settingsViewModel, string revitVersionYear)
        {
            ...
            string importDirectory = Path.Combine(AppDataDirPath, "Autodesk", "Revit", "Addins", _revitVersionYear, "TeslaRevitTools");
        ...
        public void UpdateTheTools()
        {
            if(SettingsViewModel.IsUpdateAvailable)
            {
                ...
                string manifestPath = Path.Combine(AppDataDirPath, "Autodesk", "Revit", "Addins", _revitVersionYear, "TeslaRevitTools.addin");
        ...
        private void CreateManifestFile(string manifestFilePath)
        {
            ...
            var resourceName = "TeslaRevitTools.Resources.TeslaRevitTools.txt";
        ...
        private async Task DownloadFiles(string importDirectory, string version, bool includeJson)
        {
            ...
            string dll1DownloadPath = string.Format("http://www.yourapp.pl/downloads/tesla/TeslaRevitTools/TeslaRevitTools{0}.dll", version);
            string dll1SavePath = Path.Combine(_downloadDirectory, "TeslaRevitTools.dll");

            string dll2DownloadPath = string.Format("http://www.yourapp.pl/downloads/tesla/TeslaRevitTools/CustomRevitTools{0}.dll", version);
            string dll2SavePath = Path.Combine(_downloadDirectory, "CustomRevitTools.dll");

            string jsonDownloadPath = "http://www.yourapp.pl/downloads/tesla/TeslaRevitTools/cleaner_settings.json";
